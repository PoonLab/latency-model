            ...
            if (isSampleEvent[ih + 1]) {
                ...
                if(tryCatch({
                  ...
                  v <- sample(extantLines, size = 1, prob = probstates[, 
                    l])
                  F
                }, error = function(e) T)) return(run1(repl))
                ustates[u, ] <- mstates[u, ]
                ustates[v, ] <- mstates[v, ]
...
params.list <- as.data.frame(as.list(params.list))
...
pop.list <- get.steady.state(as.list(params.list))

tfgy <- make.fgy(start.time, end.time, births, deaths, nonDemeDynamics, pop.list, migrations=migrations, parms=params.list, fgyResolution=1000, integrationMethod = "adam")
...
trees <- simulate.binary.dated.tree.fgy(tfgy[[1]], tfgy[[2]], tfgy[[3]], tfgy[[4]], sampleTimes, sampleStates, n.reps=100, integrationMethod="adam")
...
suppress<-lapply(1:100, function(i) write.tree(trees[[i]], sprintf("../data/trees/tree.%03d.tre", i)))
cat("\n")